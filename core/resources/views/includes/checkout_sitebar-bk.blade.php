      ...
      <div class="padding-top-2x hidden-lg-up"></div>
        ...
        <section class="card widget widget-featured-posts widget-order-summary p-4">
            ...
            @if ($free_shipping)
                ...
                @endif
            @elseif($shipping && $shipping['success'])
            <p class="free-shippin-aa">
              <em>
                {{ __('Shipping Charges') }} 
                <span class="set__shipping_price">{{$shipping['rate']['Postage']['Rate']}} </span>
                $
              </em>
            </p>
            @endif
            ...
            <table class="table">
              ...
              <tr class="{{Auth::check() && Auth::user()->state_id ? '' : 'd-none'}} set__state_price_tr">
                <td>{{__('State tax')}}:</td>
                <td class="text-gray-dark set__state_price"><!---{{PriceHelper::setCurrencyPrice(Auth::check() && Auth::user()->state_id ? Auth::user()->state->price : 0)}}--->
                <?php 
                    if( Auth::check() ) {
                        ?>
                            {{Auth::user()->state->type=='fixed'?'$':''}}
                            {{Auth::check() && Auth::user()->state_id ? Auth::user()->state->price : 0}}
                            {{Auth::user()->state->type=='percentage'?'%':''}}        
                        <?php
                    }
                ?>
              ...
              <tr>
                <td>{{__('Shipping')}}:</td>
                <td class="text-gray-dark set__shipping_price">{{PriceHelper::setCurrencyPrice($shipping['success'] ? $shipping['rate']['Postage']['Rate'] : 0)}}</td>
              </tr>
              @endif
              <tr>

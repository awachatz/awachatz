...
class PriceHelper
{
    ...
    public static function OrderTotal($order, $trns = null)
    {
        ...
        if (json_decode($order->shipping)) {
            ...
            if (isset($shipping['RateV4Response'])) {
                $shippingCost = $shipping['RateV4Response']['Package']['Postage']['Rate'];
            } elseif (isset($shipping['price'])) {
                $shippingCost = $shipping['price'];
            }

            //Log::info(print_r($shipping, true));
        }
        ...
        $grand_total = $cart_total + $shippingCost + $total_tax;
    ...
    public static function OrderTotalChart($order)
    {
        ...
        $shippingCost = 0;
        if ($shipipng = json_decode($order->shipping) && isset($shipping['RateV4Response'])) {
            $shippingCost = $shipping['RateV4Response']['Package']['Postage']['Rate'];
        }
    ...
    public static function getShippingService($order)
    {
        $shipping = json_decode($order->shipping, true);
        if (isset($shipping['RateV4Response'])) {
            return $shipping['RateV4Response']['Package']['Postage']['MailService'];
        }
        return 'NA';
    }
}
